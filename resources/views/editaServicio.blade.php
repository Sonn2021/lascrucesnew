        ...
        <div class="card w-100 shadow">
            ...
            <div class="card-body p-4">
                <form action="{{route('servicio.submit')}}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <input type="hidden" name="identificador" value="{{$servicio->id}}">
                    <label for="name" class="form-label">Nombre del platillo</label>
                    <input type="text" class="form-control mb-2"  name="nombre" value="{{$servicio->nombre}}" required>
                    <label for="costo" class="form-label">Costo</label>
                    <input type="number" class="form-control mb-2" name="costo" min="0" max="10000" value="{{$servicio->costo}}" required>
                    <label for="descripcion" class="form-label">Descripción del platillo</label>
                    <textarea class="form-control" class="form-control mb-5" id="exampleFormControlTextarea1" rows="3" name="descripcion" required>{{$servicio->descripcion}}</textarea>
                    ...
                    <select class="form-select mt-2 mb-2" aria-label="Default select example" name="reservacion" required>
                        ...
                        <option value="1" @if($servicio->reservacion==1) selected @endif>Si</option>
                        <option value="0" @if($servicio->reservacion==0) selected @endif>No</option>
                    </select>
                    <div class="mb-3">
